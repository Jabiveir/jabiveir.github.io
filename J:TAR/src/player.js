...
export default class Player {
  Spawn(Ctx, Level, Mobile) {
    let Temp = Mobile || this.PData.Mobile;
    this.PData = {
      XPos: 24,
      YPos: 436,
      XVel: 0,
  ...
  Update(Ctx) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (this.PData.Jumping) {
        if (this.PData.OnGround) {
          if (this.PData.OnFallingBlock) this.PData.YVel = 3100 / 16.7 / 4;
          else this.PData.YVel += 3100 / 16.7 / 4;
